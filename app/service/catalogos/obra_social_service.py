...
class ObraSocialService:
    ...
    def get(obra_social_id: int) -> ObraSocialItem | None:
        resp = get_client().get(_url(f"/{obra_social_id}"))
    ...
    def update(
        *,
        ...
        resp = get_client().patch(_url(f"/{obra_social_id}"), json=payload)
        if resp.status_code == 404:
    ...
    def delete_logico(obra_social_id: int) -> None:
        resp = get_client().patch(_url(f"/{obra_social_id}"), json={"activo": False})
        if resp.status_code == 404:
            raise ValueError(f"No existe obra_social_id={obra_social_id}")
        resp.raise_for_status()
    ...
    def restore(obra_social_id: int) -> None:
        resp = get_client().patch(_url(f"/{obra_social_id}"), json={"activo": True})
        if resp.status_code == 404:
            raise ValueError(f"No existe obra_social_id={obra_social_id}")
        resp.raise_for_status()
